...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史vLLM服務與跨區監控。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

latest映像不固定版本；running降至0不證明所有curl成功。5分鐘rate不是峰值benchmark吞吐；KVusage歸零不能直接證明prefix cache全部清空。

## 程式／設定與來源

本次核對：[runtime/vllm/runtime.py](<../../runtime/vllm/runtime.py>)、[helm/vllm/values.yaml](<../../helm/vllm/values.yaml>)、[helm/vllm/templates/deployment.yaml](<../../helm/vllm/templates/deployment.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day3-vLLM-Inference-Runtime.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
8.98
```

Taiwan監控→Singapore單L4歷史架構，IP曾由.9更換；scale0舊操作不證明目前雲端已關閉。
