...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史GPU功能驗證。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

現行runtime.py已改成class並import runtime.base；舊overlay只掛runtime.py等平面檔，未掛base.py/套件結構，不能保證舊直接python /runtime/runtime.py入口仍可用。初始化及一次matmul不是training benchmark。

## 程式／設定與來源

本次核對：[runtime/pytorch/runtime.py](<../../runtime/pytorch/runtime.py>)、[kustomize/overlays/gpu-sg/kustomization.yaml](<../../kustomize/overlays/gpu-sg/kustomization.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day1—PyTorch-GPU-Runtime.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
Result Device: cuda:0
```

此舊log對應當時P100實作；現行最後sleep不是持續GPU負載。
