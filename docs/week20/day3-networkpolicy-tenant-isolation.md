...
**已核對本課程式／設定、文內操作與引用結果；證據層級：20260921隔離Calico驗收。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

驗收只測隔離namespace的ingress標籤allow/deny與恢復，沒有測主平台default-deny+DNS+same-namespace組合。DNS規則選整個kube-system的53，不等於只准DNS Pod；APIserver/外部依賴需另設allow。

## 程式／設定與來源

本次核對：[k8s/security/network-policy/allow-client-to-server.yaml](<../../k8s/security/network-policy/allow-client-to-server.yaml>)、[k8s/security/network-policy/allow-dns.yaml](<../../k8s/security/network-policy/allow-dns.yaml>)、[k8s/security/network-policy/allow-same-namespace.yaml](<../../k8s/security/network-policy/allow-same-namespace.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<../evidence/network-policy-validation-20260921.json>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
"denied_client_output": "wget: download timed out"
```

原JSON：policy前皆通、policy後denied timeout、刪除後恢復；CPU叢集已destroy，主環境enforcement仍關。

**仍缺的證據／不能證明的事：** 結構化驗收 JSON、日期與隔離環境已保存；未驗證主叢集 enforcement、egress、DNS 或完整跨租戶規則。
