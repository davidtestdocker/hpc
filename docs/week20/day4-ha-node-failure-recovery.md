...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史整組重建，非最終工作恢復。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

JobsReady只能證明重建到ready，不能證明原MPI工作成功／checkpoint續跑。cordon非node crash；nvidia-smi後sleep腳本無set-e可掩蓋前命令失敗。新工作成功與舊TAS webhook阻擋必須分開。

## 程式／設定與來源

本次核對：[k8s/gpu-scheduling/examples/jobset-mpi.yaml](<../../k8s/gpu-scheduling/examples/jobset-mpi.yaml>)、[k8s/recovery/gpu-node-failure-test.yaml](<../../k8s/recovery/gpu-node-failure-test.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<day4-ha-node-failure-recovery.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
Restarts Count Towards Max: 1
```

歷史RestartJobSet1次；20260922另有獨立worker停啟接續驗收，不是nodefailover或Redis全失恢复。
