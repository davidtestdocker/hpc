...
**已核對本課程式／設定、文內操作與引用結果；證據層級：選型概念，未部署技術。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

OpenStack/HTCondor/LSF/DLRover未在repo部署；架構圖不是必經串接，選型表不是唯一選擇。reservation不是無條件開始時間保證；elastic6/8workers是概念，不是已驗證或任意配置可自動恢復。
...
來源：[記錄／示例原文](<day6-ai-hpc-platform-technology-selection.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
condor_submit job.sub
```

命令是示意，不是執行結果；此課沒有獨立實測輸出，不拿已做K8s／MPI成果代替其他產品經驗。
