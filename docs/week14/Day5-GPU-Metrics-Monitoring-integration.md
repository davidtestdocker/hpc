...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史DCGM故障與metrics。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

TCP5555可連不等於DCGM協定／版本相容。hostengine使用privileged與hostPort、exporter加SYS_ADMIN，不可描述成最小權限設定。

## 程式／設定與來源

本次核對：[benchmark/k8s/dcgm-exporter-remote.yaml](<../../benchmark/k8s/dcgm-exporter-remote.yaml>)、[benchmark/k8s/nvidia-dcgm.yaml](<../../benchmark/k8s/nvidia-dcgm.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day5-GPU-Metrics-Monitoring-integration.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
29.328
```

舊P100 power29.328W與temperature51是文內結果；沒有保存完整Prometheus時序。
