...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史 metrics 摘錄。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

/job-metrics是業務JSON，/metrics是instrumentation；沒有把工作數量自動轉Prometheus gauge。in-progress 是否啟用不能僅由默认 Instrumentator()推定全部指標存在；UP只表示scrape成功，不是MPI／DB功能驗收。

## 程式／設定與來源

本次核對：[api/main.py](<../../api/main.py>)、[helm/prometheus/templates/configmap.yaml](<../../helm/prometheus/templates/configmap.yaml>)、[requirements.txt](<../../requirements.txt>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day3-FastAPI-Application-Metrics.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
process_cpu_seconds_total 0.18
```

0.18是原文示例，非目前程序CPU數據；歷史UP沒有完整target JSON。
