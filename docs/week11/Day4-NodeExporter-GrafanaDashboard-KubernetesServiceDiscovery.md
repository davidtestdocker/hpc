...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史兩target文字記錄。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

static Service DNS不因Pod IP變動就必須改設定，但只抓Service不能穩定區分所有Node。現行role為endpoints，授予EndpointSlices不表示改用該角色。DaemonSet覆蓋受selectors/taints影響；RBAC包含nodes/proxy，不能叫已最小化。

## 程式／設定與來源

本次核對：[helm/prometheus/templates/configmap.yaml](<../../helm/prometheus/templates/configmap.yaml>)、[helm/prometheus/templates/clusterrole.yaml](<../../helm/prometheus/templates/clusterrole.yaml>)、[helm/prometheus/templates/clusterrolebinding.yaml](<../../helm/prometheus/templates/clusterrolebinding.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day4-NodeExporter-GrafanaDashboard-KubernetesServiceDiscovery.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
2 / 2 UP
```

2/2是舊target摘要，不證明現在GKE監控在線；Grafana dashboard檔案不是實際時序資料。
