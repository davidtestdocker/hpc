...
程序 RSS、檔案 cache、GPU allocated 是不同量。程序被 OOM 終止要檢查容器 limit 與事件，不能只看 node 還有 free memory 就否定容器限制。
...
另有[可渲染的原版 Markdown](<../history/20260922-before-current/week12/Day2-Linux-Memory-Performance-Analysis.md>)；僅校正該副本搬移後的相對連結。下面正文原樣保留，沒有縮寫或刪掉输出。
