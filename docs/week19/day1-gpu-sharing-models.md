...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史單L4 sharing配置。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

兩Pod Running與allocatable4支持排程分享，不證明公平分時、VRAM隔離或兩份kernel同時成功。MPS是延伸未實測，quota1不需要變2即可學CPU分散式。
...
來源：[記錄／示例原文](<day1-gpu-sharing-models.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
Limit: 1
```

舊更新失敗QUOTA_EXCEEDED與maxSurge0處理保留；不把4share寫成4GPU。
