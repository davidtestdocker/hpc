...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史Kueue preemption事件。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

PriorityClass還設定PreemptLowerPriority，Kubernetes Pod preemption與Kueue整組quota eviction是兩層；此事件只支持後者案例。單namespace兩priority測試不是完整多租戶隔離／fair-share驗收。

## 程式／設定與來源

本次核對：[k8s/gpu-scheduling/clusterqueue.yaml](<../../k8s/gpu-scheduling/clusterqueue.yaml>)、[k8s/gpu-scheduling/priorityclasses.yaml](<../../k8s/gpu-scheduling/priorityclasses.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<day4-priority-preemption-multi-tenancy.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
EvictedDueToPreempted
```

low4Pod被移除、high2Pod Running為歷史摘要；沒有checkpoint或low重新成功完成證據。
