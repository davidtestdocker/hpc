...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史Kueue准入摘要。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

現行flavor是l4-timesharing-tas，queue同時管理CPU3與GPUshares4，不是原文僅GPU。admission與PodsReady不等於GPU計算完成；Kueue名義quota不是雲端GPUquota或物理容量自動偵測。
...
來源：[記錄／示例原文](<day2-kueue-gpu-admission.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
All pods reached readiness and the workload is running
```

原文Running證明當時准入／啟動，不是benchmark结果；controller現行有system pool，不沿用移除taint方式。
