...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史fio結果檔。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

readwrite是混合順序I/O，不是randrw，4k也不使它自動變random。測/tmp container storage，不是Postgres PVC；direct1降低pagecache影響，不排除所有儲存快取。

## 程式／設定與來源

本次核對：[benchmark/storage/run_fio.sh](<../../benchmark/storage/run_fio.sh>)
...
20260810摘要：read1128/write1122 IOPS、平均437.99/444.73us；原完整fio JSON未保存。
