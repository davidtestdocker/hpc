...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史同節點iperf摘要。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

同節點不能代表NIC跨node/RDMA頻寬。10367 retransmissions值得調查，throughput高不等於「網路正常」；sender≈receiver不證明無loss或latency合格。
...
來源：[記錄／示例原文](<../../benchmark/network/results/iperf3_20260810.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
18.8 Gbits/sec
```

20260810文內結果18.8Gbps、65.7GBytes、Retr10367；未保存tcpdump與完整iperf JSON。
