...
CPU 壓力工具可驗證負載生成與系統反應，但工具的 synthetic score 不等於業務效能。threads 與實際可用 CPU 配額不一致時，結果可能反映節流。
...
另有[可渲染的原版 Markdown](<../history/20260922-before-current/week13/day7-1-cpu-benchmark.md>)；僅校正該副本搬移後的相對連結。下面正文原樣保留，沒有縮寫或刪掉输出。
