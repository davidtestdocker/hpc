...
展示兩個不同 failure domains：MPI child Job failure 後的 JobSet 整組 recovery，以及 node 不可排程時的 Kueue admission blockage。本文件整理 [歷史 recovery 紀錄](../week20/day4-ha-node-failure-recovery.md)，不執行新的 failure injection。
...
| Evidence | 支持的結論 |
|---|---|
| [歷史 recovery 紀錄](../week20/day4-ha-node-failure-recovery.md) | mpi-real worker failure、restarts=1、RestartJobSet、JobsReady；另有 cordon／uncordon admission 觀察 |
