...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史 validate／plan 摘錄。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

目前 dev/main.tf 是 GKE/network/firewall，不是單台 VM。Plan 非 apply；state 可包含已匯入而非自己新建資源。讀同 root 的 .tf，不會遞迴自動載入所有目錄。

## 程式／設定與來源

本次核對：[terraform/environments/dev/main.tf](<../../terraform/environments/dev/main.tf>)、[terraform/modules/compute/main.tf](<../../terraform/modules/compute/main.tf>)、[terraform/modules/compute/variables.tf](<../../terraform/modules/compute/variables.tf>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day2-Terraform-Language-Foundation.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
1 to add
```

1 add 是當時預覽，不證明已建立 VM，也不能套用成目前 plan。
