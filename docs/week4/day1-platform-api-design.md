...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史 API 回應摘錄。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

APP_NAME 設定 OpenAPI title，但現行首頁 message 另行寫死，兩者不能混用。requirements.txt 只有部分依賴固定版本，不能稱完整可重現鎖檔。API→監控→分析整條鏈是當時規劃，不代表全部已串接。

## 程式／設定與來源

本次核對：[api/main.py](<../../api/main.py>)、[requirements.txt](<../../requirements.txt>)、[docker/Dockerfile](<../../docker/Dockerfile>)、[compose.yaml](<../../compose.yaml>)

## 已有結果與解讀

來源：[記錄／示例原文](<day1-platform-api-design.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
"message": "HPC AI Performance Engineering Platform API"
```

這是舊教材保存的根路由 message；現行 root() 回傳 HPC API DEV。它不是現在服務的 HTTP 回應，本次沒有啟動容器。

**仍缺的證據／不能證明的事：** 舊文沒有獨立 HTTP capture、映像 digest 或完整 build log；Swagger 可用的敘述不等於目前全部外部依賴健康。
