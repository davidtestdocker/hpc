...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史排程資源爭用。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

Pending也可能是image/volume等，需看PodScheduled與events；本次才是資源不足。三種scheduler是觀念對照，未實作Slurm與Ray統一提交。後來Kueue新增不抹掉當時未啟用的紀錄。
...
來源：[記錄／示例原文](<day6-cluster-scheduling-integration.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
3.0/3.0 CPU
```

3CPU滿載加1pending是Ray邏輯資源狀態，不是實體CPU100%的監控數據。
