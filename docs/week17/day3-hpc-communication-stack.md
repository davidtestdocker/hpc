...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史RDMA不可見檢查。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

工具無device只能說此環境未呈現RDMA裝置，可能受driver/虛擬化/權限影響，不能普遍斷言宿主無硬體。MPI是interface標準；TCP/RDMA與fabric的圖是簡化非唯一堆疊。
...
來源：[記錄／示例原文](<day3-hpc-communication-stack.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
ib_core 507904 0
```

hpc-demo看見Virtio與ib_core，沒有RDMA裝置紀錄，不宣稱RoCE性能或GPUDirect實測。
