...
先讀本週 README 的基礎解說，再依上方順序進入本課。目標是理解「Secret 與身份」，並能把概念對到實際檔案；第一次不要求先懂完整平台架構。
...
2. 只讀 worker envFrom 的 secretRef，再看 RBAC binding 的 subject；用文字區分登入 DB 與呼叫 Kubernetes 的身份，勿執行 get secret -o yaml。
3. 記下你的觀察與理由，區分「從程式讀到」「本機執行看到」「歷史證據記錄」。沒有做過的實驗不要填成功數值。

```bash
sed -n '39,44p' 'helm/api/templates/worker.yaml'
```
...
[改寫前完整教材快照](<../history/20260922-before-current/week7/Day2_Secret.md.txt>)保存原有教學、命令、輸出和版本註記，作為文字檔閱讀；它不是現行操作手冊。日期與環境仍依原文，不把舊結果改名成新驗收。保存規則與 SHA-256 見[歷史索引](../history/20260922-before-current/README.md)。
