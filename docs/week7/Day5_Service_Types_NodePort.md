...
**已核對本課程式／設定、文內操作與引用結果；證據層級：歷史 NodePort HTTP 摘錄。** 這是文件核對，不是重跑環境；沒有要求你再開 VM 或做本機測試。全套進度見[逐篇稽核清單](../audits/curriculum-content-audit.md)，尚未核對的頁面不算完成。

## 概念解說與現行差異

10.140.0.2 是歷史私有位址，NodePort 不代表 Internet 已可達；還需路由／防火牆。localhost:30080 的舊結果依測試位置與資料平面而定，不能作通用入口。主 overlay 是 ClusterIP。port-forward 是到所選 Pod 的除錯轉送，不是完整外部 Service 流量驗收。

## 程式／設定與來源

本次核對：[k8s/api-service.yaml](<../../k8s/api-service.yaml>)、[loadtest/benchmark.js](<../../loadtest/benchmark.js>)

## 已有結果與解讀

來源：[記錄／示例原文](<Day5_Service_Types_NodePort.md>)。下面逐字摘錄來源中的內容；它是輸出、程式或命令示例，依本頁證據層級區分，不一律視為實測。

```text
"redis": "connected"
```

舊 JSON 表示當時 Redis health 回覆；缺 headers、來源位置與防火牆條件，不作現行服務保證。
